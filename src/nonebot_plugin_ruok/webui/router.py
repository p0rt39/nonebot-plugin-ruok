...
def create_webui_router(config: ScopedConfig, data_dir: Path) -> APIRouter:
    ...
    async def _webui_guard(request: Request) -> Any:
    ...
    async def action_confirm(session_id: str) -> HTMLResponse:
        try:
    ...
    async def action_solve(session_id: str) -> HTMLResponse:
        try:
    ...
    async def action_ignore(session_id: str) -> HTMLResponse:
        try:
    ...
    async def action_link(session_id: str, other_id: str = Form(...)) -> HTMLResponse:
    ...
    async def action_unlink(session_id: str) -> HTMLResponse:
        unlink_session(data_dir, session_id)
        return HTMLResponse("<p>已解除关联</p>")

    @router.post("/ruok/_actions/link/{session_id}/remove/{other_id}")
    async def action_unlink_other(session_id: str, other_id: str) -> HTMLResponse:
    ...
    async def action_module_delete(name: str, request: Request) -> HTMLResponse:
        try:
    ...
    async def action_module_edit_form(name: str) -> HTMLResponse:
        try:
            mod = get_module(data_dir, config, name)
            if mod is None:
                return HTMLResponse("<p>Module not found</p>", status_code=404)
    ...
    async def action_notification_delete(request: Request, name: str) -> HTMLResponse:
    ...
    async def action_notification_edit_form(name: str) -> HTMLResponse:
